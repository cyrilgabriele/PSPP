...
// Minor adaptions, e.g. error detection by Karl Rege
...
void testCudaResult(const char* msg, cudaError_t err) {
	if (err != cudaSuccess) {
		fprintf(stderr, "ERROR: %s (error code %d)\n%s", msg, err, cudaGetErrorString(err));
		exit(EXIT_FAILURE);
	}
}
...
int main(int argc, char* argv[]) {
    ...
    printf("\nCALCULATION OF MONTECARLO PI with %d iterations\n",BLOCKS*THREADS*TRIALS_PER_THREAD);
	printf("# of trials per thread = %d, # of blocks = %d, # of threads/block = %d.\n", TRIALS_PER_THREAD,
		BLOCKS, THREADS);

    start = clock();
    cudaFree(0); // to init device
   
	err = cudaMalloc((void**)&dev, BLOCKS * THREADS * sizeof(float)); // allocate device mem. for counts
	testCudaResult("cudaMalloc", err);
