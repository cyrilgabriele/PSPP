...
modul(dd1).
bezeichnung(dd1,'Digitale Daten¸bertragung 1').
voraussetzung(sisy1,dd1).
ectsPunkte(dd1,4).
semester(dd1,5).

modul(dd2).
bezeichnung(dd2,'Digitale Daten¸bertragung 2').
voraussetzung(dd1,dd2).
ectsPunkte(dd2,4).
semester(dd2,6).

modul(ood).
bezeichnung(ood,'Objektorienterte Analyse und Design f¸r Informatiker').
...
modul(jff).
bezeichnung(jff,'Java f¸r Fortgeschrittene').
...
% M Modul
% V Voraussetzung
voraussetzungen(V,M) :- voraussetzung(V,M).
voraussetzungen(V,M) :- voraussetzung(V,Z),voraussetzungen(Z,M).

%Aufgabe 2
% M Modul
% W Wahlbares Modul
waehlbar(W,M) :- voraussetzungen(M,W).

%Aufgabe 3
% M Modul
% L Liste der wählbaren Module
waehlbarList(L,M) :- setof(W,waehlbar(W,M),L).
